  ...
  ros::Time current_ros_time = ros::Time::now();
    ...
    {        
        ...
        if ( data->m_greyscale_image_left[CAMERA_ID] ){
          ...
          left_8.header.stamp = current_ros_time;
          ...
          g_cam_info_left.header.stamp = current_ros_time;
          ...
          try {
            ...
            ROS_ERROR("Load rgith param yaml file failed.");
        ...
        if ( data->m_greyscale_image_right[CAMERA_ID] ){
          ...
          right_8.header.stamp	 = current_ros_time;
          ...
          g_cam_info_right.header.stamp = current_ros_time;
          ...
          try {
            ...
            // if yaml fails to read data, don't try to publish
            ROS_ERROR("Load left param yaml file failed.");
          }
        }
        if ( data->m_depth_image[CAMERA_ID] ){
          ...
          // depth_16.header.stamp	  = current_ros_time;
          // depth_16.encoding	  = sensor_msgs::image_encodings::MONO16;
          // depth_image_pub.publish(depth_16.toImageMsg());
          sensor_msgs::ImagePtr depth16UC1_msg( new sensor_msgs::Image );
          depth16UC1_msg->header.frame_id   = "guidance";
          depth16UC1_msg->header.stamp   = current_ros_time;
          depth16UC1_msg->height   = 240;
          depth16UC1_msg->width    = 320;
          depth16UC1_msg->encoding = sensor_msgs::image_encodings::TYPE_16UC1;
          depth16UC1_msg->step     = 320 * (sensor_msgs::image_encodings::bitDepth(depth16UC1_msg->encoding) / 8);
          depth16UC1_msg->data.resize(depth16UC1_msg->height * depth16UC1_msg->step);
          // Fill in the depth image data, converting mm to m
          uint16_t bad_point = 0;
          const uint16_t* raw_data = reinterpret_cast<const uint16_t*>(&g_depth.data[0]);
          uint16_t* depth_data = reinterpret_cast<uint16_t*>(&depth16UC1_msg->data[0]);
          for (unsigned index = 0; index < depth16UC1_msg->height * depth16UC1_msg->width; ++index)
            {
              uint16_t raw = raw_data[index];
              depth_data[index] = (raw == 0) ? bad_point : (uint16_t)(raw / pow(2.0, 7) * 1000);
            }

          depth_image_pub.publish(depth16UC1_msg);
        }
    ...
    if ( e_imu == data_type && NULL != content )
    {
        ...
        g_imu.header.stamp    = current_ros_time;
    ...
    if ( e_velocity == data_type && NULL != content )
    {
        ...
        g_vo.header.stamp    = current_ros_time;
    ...
    if ( e_obstacle_distance == data_type && NULL != content )
    {
        ...
        g_oa.header.stamp    = current_ros_time;
        for ( int i = 0; i < CAMERA_PAIR_NUM; ++i )
    ...
    if ( e_ultrasonic == data_type && NULL != content )
    {
        ...
        g_ul.header.stamp    = current_ros_time;
        for ( int d = 0; d < CAMERA_PAIR_NUM; ++d ){
...
int main(int argc, char** argv)
{
    ...
    depth_image_pub			= my_node.advertise<sensor_msgs::Image>("/guidance/depth/image_raw",1);
